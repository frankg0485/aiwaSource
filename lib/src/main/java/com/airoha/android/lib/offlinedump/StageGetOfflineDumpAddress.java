...
public class StageGetOfflineDumpAddress extends MmiStage {

    String TAG = "StageGetDumpAddress";

    public StageGetOfflineDumpAddress(AirohaMmiMgr mgr) {
        super(mgr);
        mRaceId = RaceId.RACE_GET_OFFLINE_DUMP_ADDR;
    ...
    public void parsePayloadAndCheckCompeted(int raceId, byte[] packet, byte status, int raceType) {
        mAirohaLink.logToFile(TAG, "RACE_GET_OFFLINE_DUMP_ADDR resp status: " + status);
