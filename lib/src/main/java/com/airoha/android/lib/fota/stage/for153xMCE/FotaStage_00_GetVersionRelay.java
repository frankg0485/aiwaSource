...
public class FotaStage_00_GetVersionRelay extends FotaStage_00_GetVersion {
    public FotaStage_00_GetVersionRelay(AirohaRaceOtaMgr mgr, byte[] recipients) {
        super(mgr, recipients);

        mRaceId = RaceId.RACE_RELAY_PASS_TO_DST;
        mRaceRespType = RaceType.INDICATION;

        mRelayRaceId = RaceId.RACE_FOTA_GET_VERSION;
    ...
    protected void passToMgr(byte[] version) {
        mOtaMgr.setPartnerVersion(version);
    }
}
